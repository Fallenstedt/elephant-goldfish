---
name: eg-brainstorm
description: Generate divergent concept ideas for a rough idea using parallel goldfish subagents, optional web research, and structured synthesis. Use this for early-stage thinking, half-formed app ideas, or strategy stress-testing.
disable-model-invocation: true
---

Brainstorm a new concept using the elephant/goldfish workflow, **inverted**: instead of one goldfish stress-testing the elephant's plan, you spawn **multiple** goldfish in parallel using the `Task` tool (with `subagent_type: "generalPurpose"`), each with a different lens, each free to be creative and pull from the web. Their lack of shared context with the elephant is the point — they generate divergent ideas, not convergent ones. The elephant then synthesizes.
...
## Step 0: Frame via `AskQuestion`

**Every question to the user in this flow goes through the `AskQuestion` tool.** No free-form chat questions during framing — structured choices keep the session moving. Free-form refinement is reserved for after the seed is drafted (Step 1).

Ask three questions in sequence using a single `AskQuestion` call with three items:

**Q1 — Stage:**
- `prompt`: "What stage is this at?"
- `options`:
...
- `prompt`: "How many concepts should the goldfish surface, total?"
- `options`:
...
**Q3 — Web research:**
- `prompt`: "Should the goldfish search the web for prior art, comparable products, sources?"
- `options`:
...
Print the seed. Then ask via `AskQuestion`:

**Q4 — Seed approval:**
- `prompt`: "Does this seed match what you wanted?"
- `options`:
...
If the user picks "Refine in chat first," ask **Q4.5** via `AskQuestion` first, to scope the correction (use `allow_multiple: true`):

**Q4.5 — Which seed field needs correction:**
- `prompt`: "Which part of the seed needs fixing?"
- `allow_multiple`: `true`
- `options`:
...
Pick **3-5 lenses** based on the seed. Each lens becomes one goldfish. They run in parallel — **execute multiple `Task` tool calls concurrently in a single message** (Cursor runs them simultaneously).
...
- `description: "Brainstorm lens: <LENS NAME>"`
- `prompt:` the body below

**Prompt body to send to each goldfish:**
...
3. <IF Q3 = "On, broad" or "On, focused">: Search the web (`WebSearch` or `WebFetch`) where it sharpens an idea: prior art, comparable products, surprising data points. Cite sources inline.
...
After all parallel goldfish return, if and only if the breadth target was "~10" or "~20," spawn ONE more goldfish via `Task` (`subagent_type: "generalPurpose"`):
...
## Step 5: Converge via `AskQuestion`
...
If **Pick a direction**: ask **Q6** via `AskQuestion`. Use the **top 3 ranked picks** from the brief as the first three options, plus two escape hatches: "A different concept from the brief" and "A hybrid or new direction".

Then ask **Q7** via `AskQuestion`:

**Q7 — Handoff:**
- `prompt`: "Want to spin the chosen concept into the `eg-new-feature` skill to start designing the build?"
- `options`:
...
If they pick yes, end this skill and tell the user: "Run `/eg-new-feature <chosen concept name>` when ready."

If **Save brief and stop**: write the concepts brief to `[BOOTSTRAP: docs path or inferred working-notes location, e.g. `docs/eg-brainstorms/<slug>-<YYYY-MM-DD>.md` or `notes/`]` ONLY if the user confirms via one more `AskQuestion` call. Use the `Write` tool to create the file.
...
**STOP.** No commit, no code changes — this skill produces a brief, not a diff.
