---
name: eg-new-feature
description: Build a new feature using the elephant/goldfish workflow — scope confirm, design doc, three-goldfish design check (comprehension + critic + readiness), implementation, review, validation. Use when the user asks to build, ship, or implement a feature, or references a GitHub issue describing one.
disable-model-invocation: true
---

Build a new feature using the elephant/goldfish workflow. The aim: design before code, let three fresh goldfish stress-test the design doc, then implement, review, and validate.

If the user gave a GitHub issue URL or `#<number>`, fetch it via the `Shell` tool with `gh issue view <number> --json title,body,labels,comments` and seed the design doc from it.
...
Print the design doc to the user. For substantial features, propose writing it to `[BOOTSTRAP: docs path]` using the `Write` tool and ask the user before creating the file.
...
If the user explicitly asks to skip the gate ("just write the code", "skip the design doc", etc.), restate the gate, name the still-open passes, and require an explicit override ("yes, override the no-code gate") before any `Write` or `StrReplace` call. The exception is the `eg-fix-bug` skill for trivial fixes covered by its own Step 0 triviality gate.

The design doc itself, test names mentioned in chat (not yet on disk), and read-only exploration (`Read`, `Grep`, `Glob`, `Shell` for `git status` / `git log` / `git diff`) are NOT code edits and are permitted.

## Step 2: Three-goldfish design check

Run the article's full design-stage protocol: three `Task` invocations per round (or two on revisions — see below), each with no prior context. The combined gate is "ready iff critic AND readiness both sign off"; comprehension is informational.

Each pass uses `subagent_type: "generalPurpose"` and gets ONLY the design doc (no chat history, no implementation intent, no other passes' output). The asymmetry is the value.

**Round 1 runs all three passes; round 2+ skips comprehension** (revisions are gap-driven, not structural — once the doc reads cleanly, it almost always still reads cleanly). On every round, run critic and readiness. **Send all three (or two) `Task` calls in a single message so they run concurrently.**
...
```
<<<DESIGN_START>>>
You are a fresh reviewer with no prior context. Below is a design doc for a feature in the [BOOTSTRAP: project name + one-sentence description] repo (AGENTS.md / CLAUDE.md at the repo root has the architecture).
...
- Are there project-specific gotchas the doc ignores? AGENTS.md / CLAUDE.md is your reference.
...
If a round is **not ready**, bundle the critic gaps and readiness open questions (and comprehension feedback if it returned unclear) into a single revise prompt with section labels (`=== CRITIC GAPS ===`, `=== READINESS OPEN QUESTIONS ===`, `=== COMPREHENSION FEEDBACK ===`). Tell the elephant to address EVERY numbered gap across BOTH the CRITIC GAPS and READINESS OPEN QUESTIONS sections — do not collapse or skip because the numbering restarts. Each gap is either: addressed in a doc revision, or rebutted with a verbatim reason citing AGENTS.md / CLAUDE.md or the user's words. Print the revised doc back to the user once both gates close.

Then re-run Pass B and Pass C against the revised doc (skip Pass A — see above). If the round still does not converge after **three revisions**, the feature is under-specified — **stop and ask the user** for more direction via `AskQuestion`.
...
Follow the plan using `StrReplace`, `Write`, and the `Shell` tool. After each layer, briefly verify before moving on:
...
Run the `eg-precommit-review` skill explicitly. The reviewer is itself a goldfish; it sees only the diff, not the conversation.
...
All required tiers must pass. For UI features, also do a final walkthrough of the golden path AND the most plausible edge case (use Cursor's `browser` subagent for web, simulator/device for mobile, curl/test commands for backend).
